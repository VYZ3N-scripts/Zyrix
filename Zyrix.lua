...
--[[
════════════════════════════════════════════════════════════════════════════════
 EXAMPLE USAGE — paste this into your own loader script
════════════════════════════════════════════════════════════════════════════════

-- 1. Load the library
local Zyrix   = loadstring(game:HttpGet("YOUR_RAW_GITHUB_URL"))()
local ZyrixUI = getgenv().ZyrixUI   -- always grab AFTER loading

-- 2. Configure the key system
local cachedKeys = nil
local PASTEBIN   = "https://pastebin.com/raw/YOUR_PASTEBIN_ID"

Zyrix.Appearance.Title    = "My Script"
Zyrix.Appearance.Subtitle = "Enter your key to continue"
Zyrix.Links.GetKey        = PASTEBIN
Zyrix.Links.Discord       = "https://discord.gg/yourserver"

-- Pastebin key check (one key per line, caches after first fetch)
Zyrix.Callbacks.OnVerify = function(key)
    if not cachedKeys then
        local ok, result = pcall(game.HttpGet, game, PASTEBIN)
...
-- 3. Build GUI after key is accepted
Zyrix.Callbacks.OnSuccess = function()

    -- ALWAYS call Open() first to build the shell
    ZyrixUI:Open()

    -- ── Combat tab ───────────────────────────────────────────────────────────
    local Combat = ZyrixUI:AddTab("Combat")

    Combat:Section("Aimbot")

    local aimbotToggle = Combat:Toggle("Aimbot", nil, false, function(enabled)
        -- your aimbot enable/disable logic here
        print("Aimbot:", enabled)
    end)

    Combat:Slider("Smoothness", 1, 100, 50, "units", function(val)
        print("Smooth:", val)
    end)

    Combat:Slider("FOV", 10, 500, 120, "studs", function(val)
        print("FOV:", val)
    end)

    Combat:Section("Keys")

    local aimKey = Combat:Keybind("Aim Key", Enum.KeyCode.Q, function(key)
        print("New aim key:", key)
    end)

    Combat:Section("Targeting")

    local teamFilter = Combat:Dropdown(
        "Team Filter",
        {"All", "Enemies", "Allies"},
        "All",
        function(val)
            print("Team filter:", val)
        end
    )
    ...
    -- ── Player tab ───────────────────────────────────────────────────────────
    local Player = ZyrixUI:AddTab("Player")

    Player:Section("Movement")

    local speedToggle = Player:Toggle("Speed Hack", "Increases walk speed", false, function(v)
    ...
    local speedSlider = Player:Slider("Walk Speed", 16, 500, 16, "studs/s", function(v)
        local char = game.Players.LocalPlayer.Character
        if char and char:FindFirstChild("Humanoid") then
    ...
    Player:Toggle("Infinite Jump", nil, false, function(v)
        getgenv().__InfJump = v
        if v and not getgenv().__InfJumpConn then
            getgenv().__InfJumpConn = game:GetService("UserInputService").JumpRequest:Connect(function()
                if not getgenv().__InfJump then return end
                local char = game.Players.LocalPlayer.Character
                if char and char:FindFirstChild("Humanoid") then
    ...
    Player:Toggle("Noclip", nil, false, function(v)
        getgenv().__Noclip = v
        if v and not getgenv().__NoclipConn then
            getgenv().__NoclipConn = game:GetService("RunService").Stepped:Connect(function()
                if not getgenv().__Noclip then return end
                local char = game.Players.LocalPlayer.Character
                if char then
    ...
    -- ── Visual tab ───────────────────────────────────────────────────────────
    local Visual = ZyrixUI:AddTab("Visual")

    Visual:Section("ESP")

    Visual:Toggle("Player ESP", nil, false, function(v)
        -- your ESP logic here
        print("ESP:", v)
    end)

    Visual:Toggle("Name Tags", nil, false, function(v)
        print("Name tags:", v)
    end)

    Visual:Toggle("Health Bars", nil, false, function(v)
        print("Health bars:", v)
    end)

    Visual:Section("World")

    Visual:Toggle("Fullbright", "Max game lighting", false, function(v)
        local Lighting = game:GetService("Lighting")
        Lighting.Brightness = v and 2 or 1
        Lighting.ClockTime  = v and 14 or 14
    end)

    Visual:Toggle("No Fog", nil, false, function(v)
        game:GetService("Lighting").FogEnd = v and 1e9 or 100000
    end)

    -- ── Misc tab ─────────────────────────────────────────────────────────────
    local Misc = ZyrixUI:AddTab("Misc")

    Misc:Section("Script Executor")

    -- Working multiline executor — turns green on success, red on error
    local executor = Misc:Executor("-- Type Lua code here\nprint('Hello from Zyrix!')")

    Misc:Section("Game")

    Misc:Button("Copy Game ID", function()
        pcall(setclipboard, tostring(game.PlaceId))
        Zyrix:Notify("Copied", "Game ID copied to clipboard", 2, "copy")
    end)
    ...
    Misc:Dropdown("Server Region", {"Auto", "US East", "US West", "EU", "Asia"}, "Auto", function(v)
        print("Region:", v)
    end)

    -- ── Settings tab ─────────────────────────────────────────────────────────
    local Settings = ZyrixUI:AddTab("Settings")

    Settings:Section("Interface")

    Settings:Keybind("Toggle GUI", Enum.KeyCode.RightShift, function(key)
        getgenv().__ZyrixToggleKey = key
        Zyrix:Notify("Keybind", "GUI toggle updated to "..tostring(key):gsub("Enum.KeyCode.",""), 2, "info")
    end)

    Settings:Section("Account")

    Settings:Label("Logged in as: "..game.Players.LocalPlayer.DisplayName)

    Settings:Button("Clear Saved Key", function()
        Zyrix:ClearSavedKey()
        Zyrix:Notify("Cleared", "Key removed. Re-enter on next launch.", 3, "info")
    end)

    Settings:TextBox("Custom Note", "Enter a note...", "", function(text)
        print("Note saved:", text)
    end)

    -- ── Reading values programmatically ──────────────────────────────────────
    -- print(aimbotToggle:GetValue())   -- true / false
    -- print(speedSlider:GetValue())    -- number
    -- print(aimKey:GetValue())         -- Enum.KeyCode
    -- print(teamFilter:GetValue())     -- string
    -- executor:Execute()               -- run executor content from code
    -- aimbotToggle:SetValue(true)      -- force a value
    -- speedSlider:SetValue(200)        -- force slider value
...
Zyrix.Callbacks.OnClose = function()
    print("Key system closed by user")
end

-- 4. Launch — shows background + key system
Zyrix:Launch()

--  NOTES:
--  • Click the Z logo in the pill bar to toggle the main window open/closed.
--  • Press RightShift to toggle (PC). Change with Settings:Keybind above.
--  • Tab bar scrolls horizontally if you have many tabs.
--  • Executor: green flash = success, red flash = error (check output/console).
--  • All element callbacks run inside pcall so one error won't break the UI.
]]
